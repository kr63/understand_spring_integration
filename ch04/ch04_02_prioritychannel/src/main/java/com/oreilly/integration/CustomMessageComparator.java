...
public class CustomMessageComparator implements Comparator<Message<String>> {
	...
	public int compare(Message<String> message, Message<String> message2) {
		...
		boolean isPayloadEven = Integer
                .valueOf(payload.substring(payload.length() - 1)) % 2 == 0;
        boolean isPayload2Even = Integer
                .valueOf(payload2.substring(payload2.length() - 1)) % 2 == 0;
